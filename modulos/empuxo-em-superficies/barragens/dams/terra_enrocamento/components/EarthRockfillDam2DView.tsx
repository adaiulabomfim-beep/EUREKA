...
import { useEarthRockfillDamSimulation } from '../hooks/useEarthRockfillDamSimulation';
...
export const EarthRockfillDam2DView: React.FC<DamRendererProps & { is3D: boolean, setIs3D: any, showVectors: boolean, setShowVectors: any }> = (props) => {
  const {
  ...
  const { worldGeometry, getDamXAtY, profile } = useEarthRockfillDamSimulation({
    damHeight, damBaseWidth, damCrestWidth, inclinationAngle,
  ...
  const vectors = useMemo(() => {
    ...
    return vecs;
  }, [showVectors, upstreamLevel, downstreamLevel, isAnalyzed, force, s_cp, up, damHeight, getDamXAtY, toWorldX, project, SCALE]);
