...
export const ArchDam3DView: React.FC<DamRendererProps & { is3D: boolean, setIs3D: any, showVectors: boolean, setShowVectors: any }> = (props) => {
  const {
  ...
  const archRadius = useMemo(() => Math.max(CHANNEL_WIDTH * 0.8, damBaseWidth * 2.0), [damBaseWidth, CHANNEL_WIDTH]);
  const archOffsetFn = useMemo(() => {
    return (z: number) => {
      const rr = archRadius * archRadius;
      const zz = z * z;
      // Make the curve more pronounced
      return (archRadius - Math.sqrt(Math.max(0, rr - zz))) * 1.5;
    };
  }, [archRadius]);
  ...
  const vectors = useMemo(() => {
    ...
    const pushArrow = (x: number, y: number, z: number, nx: number, ny: number, nz: number, magWorld: number, color: string, isResultant: boolean, val?: string) => {
      ...
      const visibilityFactor = Math.max(0, rotatedNormal.z);
      ...
      vecs.push({
        ...
        isResultant,
        val: val || "",
        opacity: (isResultant ? 1 : 0.40) * visibilityFactor,
    ...
    return vecs;
  }, [showVectors, upstreamLevel, downstreamLevel, isAnalyzed, force, s_cp, up, damHeight, getDamXAtY, toWorldX, project, rotate, SCALE, archOffsetFn]);
