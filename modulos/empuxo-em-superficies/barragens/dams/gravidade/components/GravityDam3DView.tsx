...
export const GravityDam3DView: React.FC<DamRendererProps & { is3D: boolean, setIs3D: any, showVectors: boolean, setShowVectors: any }> = (props) => {
  const {
  ...
  const vectors = useMemo(() => {
    ...
    const pushArrow = (x: number, y: number, z: number, nx: number, ny: number, nz: number, magWorld: number, color: string, isResultant: boolean, val?: string) => {
      ...
      let pStart = project({ x: x - fx * finalMag, y: y - fy * finalMag, z: z - fz * finalMag });

      if (!isInside(pStart)) {
        for (let f of [0.8, 0.6, 0.4, 0.2, 0.1, 0.05]) {
          const testMag = magWorld * f;
          const testStart = project({ x: x - fx * testMag, y: y - fy * testMag, z: z - fz * testMag });
